  ...
  */
  ...
  static async updateComment(commentId, userEmail, text, date) {
    try {
      ...
      const filter = { _id: ObjectId(commentId), email: userEmail }

      const updateResponse = await comments.updateOne(filter, {
        $set: {
          text: text,
          date: date,
        },
  ...
  static async deleteComment(commentId, userEmail) {
    ...
    */
    ...
    try {
      ...
      const filter = { _id: ObjectId(commentId), email: userEmail }
      const deleteResponse = await comments.deleteOne(filter)
